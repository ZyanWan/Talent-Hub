...
1. **PDF 页数安全限制**：先用 pypdfium2 统计页数，超过 `MAX_PDF_PAGES = 100` 页直接拒绝，记录错误 `pdf-page-limit`，不进入提取。
2. **文本层提取**：调用 `extract_file()`：
   ...
   - `.txt/.md/.markdown`：按 `utf-8-sig → utf-8 → gb18030` 顺序尝试解码，方法 `text`。
...
5. **水印行过滤**：删除"无空格、无中文、纯 ASCII 字母数字混合、长度 ≥ 15、且在全文重复出现 ≥ 2 次"的行（PDF 水印/防伪条被提取为文本的产物）。正常内容因须同时满足"混合串 + 重复出现"两个条件而几乎不受影响。
...
7. **同行断词修复**：
   ...
   - 中英混排断词、或后片段为常见英文词尾（ed / ing / estment 等 29 个）时合并：`Li sting` → `Listing`、`Inv estment` → `Investment`、`Water ing` → `Watering`。
...
`extract_file()` 与 `extract_document()` 输出以下字段：
