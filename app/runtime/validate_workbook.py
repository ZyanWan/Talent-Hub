...
def validate_loaded_workbook(wb, initial_errors: list[str]) -> tuple[list[str], list[str]]:
    ...
    recommend_candidates: set[str] = set()
    if RECOMMEND_SHEET in sheet_headers:
        ws = wb[RECOMMEND_SHEET]
        headers = sheet_headers[RECOMMEND_SHEET]
        for row_number in sheet_rows.get(RECOMMEND_SHEET, []):
            candidate = cell_text(ws, row_number, headers, "候选人")
            if not candidate or candidate not in summary_candidates:
                if candidate:
                    errors.append(f"{RECOMMEND_SHEET} has unknown candidate: {candidate}")
                continue
...
def main() -> int:
    ...
    print("PASS: workbook satisfies talent-hub structural and security contract")
