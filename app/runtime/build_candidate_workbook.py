...
def coerce_rows(rows: list[object], headers: list[str]) -> list[list[object]]:
...
def build_workbook(data: dict, output_path: Path) -> None:
    ...
    for index, sheet_name in enumerate(ordered_sheet_names):
        ...
        assert ws is not None  # openpyxl Workbook 初始化时始终包含一个工作表
...
def sample_payload() -> dict:
    return {
        ...
        RECOMMEND_SHEET: {
            "_headers": list(DEFAULT_HEADERS_PER_SHEET[RECOMMEND_SHEET]),
            "_rows": [],
        },
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build talent hub workbook.")
