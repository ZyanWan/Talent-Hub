...
- A 类条件守卫：标准定义 `a_conditions` 时，`a_conditions_check` 的判定条数必须与标准一致，且每条状态为「满足」、依据有简历事实锚点，否则程序不能判 A，降为 B 并为存疑条件生成核实电话问题（focus 前缀「A类条件核实-」）；模型未返回判定或返回空列表时按不满足处理，防止漏判导致 A 失守。标准未定义 A 类条件时该闸跳过。
