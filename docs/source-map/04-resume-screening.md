...
- 没有上述明确失败，但存在硬条件 `unknown`，核心维度不是全部“匹配”，或 A 类条件判定未全部通过 → B。
- 全部硬条件 `met`、四个核心维度全部“匹配”，且 A 类条件逐条判定全部为「满足」→ A。
...
- 守卫校验：`met/unmet` 引文与 note 均无事实锚点 → 降为 `unknown`；criteria 中的硬性门槛未被模型判定 → 按 `unknown` 补齐并告警（避免模型漏判使硬条件被默认通过）。
- A 类条件守卫：标准定义 `a_conditions` 时，`a_conditions_check` 的判定条数必须与标准一致，且每条状态为「满足」、依据有简历事实锚点，否则程序不能判 A，降为 B 并为存疑条件生成核实电话问题（focus 前缀「A类条件核实-」）；模型未返回判定或返回空列表时按不满足处理，避免漏判把不满足项判为 A。标准未定义 A 类条件时该闸跳过。
- 评估输出包含 `a_conditions_check`：对筛选标准中每条 `a_conditions` 逐条判定（满足/不满足/存疑）并给出简历依据。prompt 约束：判定条数与标准一致，任一条件不满足或存疑时结论不得判 A；只有核心能力证据来自近期经历时才可判 A（证据主要来自五年以前，或近三年主要岗位职责与岗位要求的核心职能不一致时不得判 A）；不得把协助性动作当独立负责、把试产或样品阶段经历当量产物料履行的证据来满足年限或闭环判定。
- 程序最终等级同时使用硬条件状态、四个核心维度状态和 A 类条件判定；模型输出的 `conclusion` 只作为结构输入，不拥有最终决定权。
- 模型对可选字段显式返回 `null` 时，接收端将其归一为该字段的语义默认（如 `未体现` 或空串）。
...
该顺序保证 `job.json` 不领先于实际结果文件，不可交换。Excel 校验通过后的最终 `评估结果.json` 与 `解析清单.json` 也使用同一原子写入函数，避免收尾中断破坏可续跑检查点。
