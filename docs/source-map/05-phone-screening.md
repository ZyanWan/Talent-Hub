...
电话链路只有一个 AI 整理业务阶段，不另设语义复核阶段。模型输出经 JSON 解析和基础结构校验后形成 `CallSummary`；JSON 或结构失败时最多追加一次带纠正要求的模型调用。程序不对客观记录、软性素质评价或字段状态做语义裁决。

`facts[].ref` 只用于定位录音时间，不影响 `narrative`。人工编辑是最终校正边界。`CallRepository` 复用 `repository.py` 的 `JsonStore`，电话任务与岗位任务共享 JSON 保存、归档、恢复、删除和锁的底层仓储规则。
...
frontend/src/views/PhoneView.tsx 处理任务
  ...
  → 校验后缀和 100 MB 上限
  → reserve_audio() 处理重名
  → main.py 流式写入预留路径并计算 SHA-256
  → 内容哈希重复：删除本次写入并返回 upload.accepted=false
  → 新内容：合并 record.json.audio_hashes
  → add_item()
  → record.json.items[] 新增 queued 条目
```

`title_mode` 使用 `auto|custom` 区分日期默认标题与用户标题。前端对 `auto` 标题按当前界面语言渲染，`custom` 标题保持用户原文。任务缺少该字段时，只有严格符合中英文日期默认格式的标题按自动标题显示。
...
- 完全替换语义：每次选择都替换现有勾选与自定义文本；切回「不关联岗位」时一并清空。
...
POST /api/calls/<id>/process
  → CallProcessor.start()
  → failed 条目重置为 queued 并清空 summary，done 保留
  → 后台 _run() 并发处理 queued 条目
  → _process_item()
     1. queued → transcribing
     2. transcripts/<item_id>.txt 可读且非空时复用；否则调用 transcribe_audio() 并渲染、保存转写
     3. transcribing → summarizing
     4. _validated_summarize() 生成 CallSummary；JSON 或基础结构失败时纠正一次
     5. render_remark_narrative() 渲染客观记录、软性素质评价与可选快筛详情
     6. attach_fact_timestamps() 尝试写入 facts[].start_time/end_time
     7. 保存 summaries/<item_id>.json 和 .md
     8. record.json 中对应条目写入 summary，summarizing → done
```

复用转写文本时，程序会尝试读取 `transcripts/<item_id>.json` 中的 utterances 用于时间戳定位。该 JSON 缺失或不可解析时仍复用文本，只将时间戳定位降级为文本级。

AI 以高级招聘专员的第一工作视角整理客观记录和软性素质评价。候选人信息、软性关注项和转写放入不可信数据边界。关注项只是参考，不是强制输出清单。转写超过 160,000 字符时保留首 120,000 与末 40,000 字符并标注省略；快筛问答由 `call_qa_records` 控制。

录音回放定位：`GET /api/calls/{call_id}/items/{item_id}/audio` 按 token 校验后返回录音文件流，前端 fetch 为 Blob 播放且不新增持久化存储。`attach_fact_timestamps()` 在落盘前计算并持久化 `facts[].start_time/end_time`；条目未持久化时间戳时，前端将该事实显示为不可点击。

该整理阶段在 JSON 或必填结构失败时可追加一次纠正调用，因此最多发起两轮 `chat_json()`；每轮内部仍可能执行传输重试：

| 阶段 | 模型输入 | 模型输出 | 程序责任 |
| --- | --- | --- | --- |
| 整理 | ASR 转写、可选软性关注项 | 结构化 `CallSummary`（不含 narrative；qa_records 按开关生成） | 校验基础结构，完整渲染 narrative，尝试定位事实时间 |

### 9.3 `CallSummary` 持久化结构与人工编辑边界
...
模型若返回空壳，程序视为结构失败并携带错误纠正一次；连续失败则该条目进入 failed，不以空摘要标记 done。

### 9.4 基础结构校验与录音定位

`validate_call_structure()` 只拒绝动态章节为空、章节标题或要点为空、字段列表为空的结果。它不检查预期字段是否全部覆盖，不分析文本语义，也不删改正文、软性素质评价、问答或字段状态。
...
同一电话任务中的 queued 录音最多以 5 路并发处理（`ITEM_CONCURRENCY`）：实际 worker 数为 5 与待处理条目数的较小值。每条录音独立完成“转写 → 整理 → 结构校验 → 落盘”。每个条目创建独立模型客户端，摘要文件使用独立 `item_id`，仓储通过 `update_item()` 在锁内读取最新 `record.json` 并只更新目标条目。
...
```text
draft ─process→ running ─┬─> done
                         ├─> failed
                         └─> cancelled
failed/cancelled ─process→ running
done + 新增 queued 条目 ─process→ running
```

`call_state.py` 还定义了任务级 `queued`，并将其纳入运行态集合；当前 `CallProcessor.start()` 直接把任务置为 `running`，没有单独排队阶段。

电话业务处理结束、写入任务终态前，`_push_notifications()` 按条目稳定顺序筛选通知对象。目标条目必须是 `done`、包含 `summary.narrative`、不在 `feishu_baseline_item_ids` 中，且未成功推送。

每条通知通过 `push_with_status()` 发送。仅当飞书返回 `code=0` 时，程序才原子写入 `feishu_push_status=succeeded` 和 `feishu_pushed_at`。失败条目保持 `pending`，且不阻止后续条目发送。部分条目业务失败时，已完成条目仍会尝试通知；任务业务终态仍由条目处理结果决定。
...
`call_state.py` 定义了条目级 `cancelled` 终态，但转换表没有通往该状态的入边，当前处理器不写入该状态。取消或进程中断时，中间态条目收敛为 `queued`。

重要区别：

- `CallProcessor` 最多并行处理两个电话任务；任务内录音并发遵循 9.5 节。
- 取消会中止活跃 LLM 客户端。
...
任务记录侧栏通过 `HistoryMutation` 把删除身份或归档/恢复后的服务端摘要提交给 `App`。事件命中 `state.currentCall` 时，删除会清除 `talentHub.lastCall` 与当前任务，并递增 `phoneResetSignal`。`PhoneView` 随后停止轮询、释放音频对象 URL、关闭条目详情并回到新建表单。归档与恢复会把摘要合并到当前完整任务，使追加录音和重试操作立即按最新 `archived_at` 显隐。非当前电话记录的变更只刷新侧栏，不改变主区域。

电话任务记录加载使用独立递增序号。只有最后一次用户选择的响应可以写入 `state.currentCall` 和 `talentHub.lastCall`。视图退出、工作区重置和新的选择都会使早期请求失效。最后一次请求失败时，前端清空当前电话任务、待上传文件和恢复键，并回到新建表单。
