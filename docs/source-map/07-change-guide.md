...
| 如果修改 | 必须同步检查 |
| --- | --- |
| `AppSettings` 字段或默认值 | `app/config.py` 的兼容加载、归一化和公开设置，`app/main.py` 的请求模型与设置路由，`frontend/src/ui/SettingsDialog.tsx` 的表单和 `buildPayload()`，设置验证 |
| 飞书推送逻辑（`app/feishu.py`、`push_with_status`、消息构建、脱敏、签名、重试与频控） | `app/main.py` 的 `feishu-test` / 两个 `retry-notification` API、`app/pipeline.py` / `app/runtime/phone_screening.py` 挂点、通知状态与兼容基线、消息大小保护、前端飞书测试与简历通知重试入口、电话重试 API 的界面接线状态、推送失败不改变业务状态的验证 |
| 飞书密钥或 Webhook 字段 | `config.py` DPAPI、`merged_settings` exclude 集合、`public_dict` 公开字段、前端回显、清除语义 |
| API 路径、方法或响应字段 | `app/main.py`、前端 React 组件所有调用和渲染、API 验证、发布烟测 |
| Job 状态或 stage 语义 | `pipeline.py`、仓储归档限制、`main.py` 冲突处理、前端轮询、按钮与任务记录、状态验证 |
...
| 结果/产物文件名 | `app/pipeline.py`、下载/预览路由、续跑、对比缓存、持久化任务兼容加载 |
| 检查点保存顺序 | 原子写逻辑、恢复逻辑、取消、并发和中断验证 |
| 模型调用或重试策略 | `llm.py`、取消语义、设置 timeout、筛选/电话/对比调用、错误验证 |
| `CallSummary` / `CallField` / `CallFact` / `CallQA` | 整理 prompt、基础结构校验、narrative 渲染、持久化 JSON/Markdown、编辑 API、前端展示、持久化摘要兼容加载、回放定位（`start_time`/`end_time` 与音频访问 API）、电话验证 |
| ASR 请求参数或响应结构 | `speech_to_text.py`、电话处理器、设置的 ASR 状态、STT 验证 |
| 首页 DOM ID 或 class | `frontend/src/` React 组件与 router 的节点查询和事件、`frontend/public/styles.css`、首页验证、发布烟测 |
| 前端本地存储键 | 初始化恢复、切换工具、新建任务、任务记录恢复 |
| 任务记录删除、归档、恢复或打开 | `HistoryMutation` 事件、`App` 当前任务提交与工作区重置、`lastJob` / `lastCall`、筛选与电话请求序号、归档操作显隐、任务记录集成验证 |
...
7. 检查持久化兼容：现有 `job.json`、`record.json`、结果 JSON 是否能加载和恢复。
...
4. 修改筛选标准字段：它跨越模型 prompt、人工校准、Markdown、Excel 和已持久化任务 JSON。
...
- 电话摘要只经过 JSON 和必填结构校验，程序保留通过结构校验的客观记录与软性素质评价；快筛详情仅在 `call_qa_records` 开启时保留。
- 电话事实 `ref` 只用于尝试定位录音时间，不参与正文、软性素质评价和字段状态裁决。
- 电话整理结果允许 HR 修改。
- Excel 校验只保证结构、安全和跨表一致性，不宣称业务判断必然正确。

修复简历筛选的评级分布异常（通过率偏低、A 类偏少、字段大量转为待确认）时，应先检查模型引用是否真实、Prompt 与解析是否正确，不得放宽证据边界来消除症状。电话整理质量问题通过提示词和模型输出评测处理，不新增删除或改写模型业务内容的后处理逻辑。
