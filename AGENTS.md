...
**仓库根目录 `.markdownlint.json` 是 Markdown 格式的唯一约定。**

- 关闭 MD012 / MD013 / MD024 / MD025 / MD033 / MD041：允许中文长行、内联 HTML，以及 `# 0.`~`# 6.` 这类分节编号；其余规则保持开启。
- 改动 Markdown 后运行 `npx markdownlint-cli2 "*.md" "app/**/*.md" "docs/**/*.md" "debug/**/*.md"`，受版本控制的文档应无告警。
