...
| **AI summarization** | Uses a senior-recruiter perspective to produce complete structured notes, soft-skill evaluations that prioritize the selected focus dimensions without being limited to them, and optional Q&A detail (off by default). |
| **Complete result delivery** | Code validates JSON and required structure only. It does not delete model-produced notes or soft-skill evaluations or change field status based on citations; citations are used only for audio positioning. |
