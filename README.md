...
Talent Hub 把招聘里最耗时、最容易带主观偏差的几个环节串成一个流程：判断留有依据、拿不准的自动标出、结果即拿即用。设置、任务材料与结果默认保存在本机；启用模型、ASR 或飞书功能时，相应内容会发送至所配置的服务。
...
| **AI 整理** | 以高级招聘专员口径生成面向用人部门的结构化记录、优先参考关注维度的软性素质评价与可选快筛问答（默认关闭以减少模型输出长度与整理耗时）。 |
...
| **面向 HR 的结果消息** | 简历每轮推送一条总览，含提交/成功/失败人数、A/B/C 分布及最多 5 位 A/B 候选人的一句话判定；电话按候选人逐条推送整理记录，超出单条消息容量时截断并提示回到应用查看。 |
...
| **手动重试简历通知** | 在已完成的简历筛选结果页可点击「重试飞书通知」，仅补发仍待通知的结果，并显示本次是否真实发送。电话任务页不提供手动重试入口。 |
| **测试飞书链接** | 设置中可一键发送测试消息，验证 Webhook 连通性。 |

## 技术特性

- **本地优先**：设置、原始任务材料与结果默认保存在用户数据目录（Windows：`%LOCALAPPDATA%\TalentHub`；macOS：`~/.local/share/TalentHub`），默认目录位于源码目录之外；通过 `TALENT_HUB_DATA_DIR` 或 `--data-dir` 自定义时由使用者选择位置。解析后的简历文本仅在「保留解析文本」开启时保存。
...
| 配置 | 默认值 / 范围 | 说明 |
| --- | --- | --- |
| 模型服务基础地址 | `https://api.openai.com/v1` | 兼容 Chat Completions 与 JSON 输出的 OpenAI 格式服务；填写不含 `/chat/completions` 的基础地址，应用会自动追加该路径，不强制以 `/v1` 结尾。 |
| API Key | 空 | 模型服务的访问密钥；Windows 使用 DPAPI 加密保存，macOS 使用 `TALENT_HUB_API_KEY` 环境变量。 |
| 模型名称 | 空 | 实际使用的模型标识，由模型服务商决定。 |
| 并发数 | 6（1–12） | 每批同时处理的候选人数量。提高并发可增加并行处理量，实际耗时受候选人数、模型服务延迟与限流影响；遇限流时请调低。 |
| 请求超时（秒） | 180（30–600） | 普通模型 HTTP 请求每次尝试的超时。筛选标准与单份简历的每轮模型调用分别允许最多 3 次和 2 次可重试传输尝试；JSON / 结构校验失败时可再发起一轮模型调用纠正。电话整理的每轮模型调用中，每次尝试至少使用 300 秒，允许最多 3 次可重试传输尝试；结构校验失败时也可再发起一轮。 |
| Tesseract 路径 | 空 | 处理扫描 PDF 或图片简历时的 Tesseract 程序路径；留空时从 `TESSERACT_CMD`、`PATH` 和平台常见路径自动探测。文本型 PDF/DOCX/TXT/Markdown 无需配置，中文简历建议安装 `chi_sim` 语言包。 |
| 保留解析文本 | 开启 | 是否在本机保留逐份简历的解析文本；关闭后不保存这些文本，但仍保留解析后的 JD 与不含简历正文的解析清单。 |
...
| `TALENT_HUB_DATA_DIR` | 覆盖默认数据目录（Windows：`%LOCALAPPDATA%\TalentHub`；macOS：`~/.local/share/TalentHub`），用于设置、任务材料与结果文件；解析后的 JD 会保存，解析后的简历文本仅在「保留解析文本」开启时保存。自定义路径由使用者负责避开源码目录。 |
...
- 应用数据默认保存在用户本机（Windows：`%LOCALAPPDATA%\TalentHub`；macOS：`~/.local/share/TalentHub`），包括设置、原始任务材料、解析后的 JD、结果文件、电话转写与整理档案；解析后的简历文本仅在「保留解析文本」开启时保存。
...
- 飞书推送通过 Webhook 将配置的消息内容发送至飞书服务器。程序按常见手机号、座机号和邮箱格式自动替换敏感信息，但特殊写法可能无法识别；请妥善保管 Webhook 地址，并在启用推送前确认外发内容。
