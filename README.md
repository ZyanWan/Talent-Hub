...
招聘流程中三项重复性最高、也最容易因判断尺度不一而失真的工作：批量简历初筛、判断依据的复核与说明、电话录音要点整理与结果汇总。

Talent Hub 把这些环节串成一个流程，逐条留下可核验的依据，结果可直接交付。设置、任务材料与结果默认保存在本机；启用模型、ASR 或飞书功能时，相应内容会发送至所配置的服务。
...
| `TALENT_HUB_DATA_DIR` | 覆盖默认数据目录（Windows：`%LOCALAPPDATA%\TalentHub`；macOS：`~/.local/share/TalentHub`），用于设置、任务材料与结果文件；解析后的 JD 会保存。自定义路径由使用者负责避开源码目录。 |
